...
import { getPurchaseOrders, createPurchaseOrder, receivePurchaseOrder, deletePurchaseOrder } from '../../api/purchaseOrders';
...
export default function PurchaseOrdersPage() {
    ...
    const handleSubmit = async (formData) => {
        try {
            ...
            const message = err.response?.data?.message || 'Gagal membuat Purchase Order.';
            toast.error(message);
        } finally {
            setSubmitting(false);
        }
    ...
    const handleReceive = async (order) => {
        const result = await Swal.fire({
            title: 'Terima PO ini?',
            html: `PO <strong>${order.poNumber}</strong> akan diterima.<br/>Stok produk akan bertambah.`,
            icon: 'question',
        ...
        try {
            ...
            toast.success('PO berhasil diterima. Stok telah diperbarui.');
            await fetchData();
        } catch (err) {
            const message = err.response?.data?.message || 'Gagal menerima PO.';
            toast.error(message);
        }
    };
    ...
    const handleDelete = async (order) => {
        const result = await Swal.fire({
            title: 'Hapus PO ini?',
            text: `PO "${order.poNumber}" akan dihapus.`,
            icon: 'warning',
            showCancelButton: true,
        ...
        try {
            ...
            const message = err.response?.data?.message || 'Gagal menghapus PO.';
            toast.error(message);
        }
    };
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredOrders.map((order) => (
                                <tr key={order.id} className="hover:bg-gray-50">
                                    <td className="px-4 py-3 text-sm font-mono font-medium text-gray-800">{order.poNumber}</td>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center p-6 border-b border-gray-200">
                            <h2 className="text-xl font-bold text-gray-800">Detail PO</h2>
                        ...
                        <div className="p-6 space-y-4">
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    ...
                                    <p className="font-medium">{detailOrder.poNumber}</p>
                                </div>
                                <div>
                                    <p className="text-gray-500">Supplier</p>
                                    <p className="font-medium">{detailOrder.supplierName}</p>
                                </div>
                                <div>
