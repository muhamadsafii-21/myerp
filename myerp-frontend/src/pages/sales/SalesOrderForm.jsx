...
export default function SalesOrderForm({ isOpen, onClose, onSubmit, customers, products, loading }) {
    ...
    const hasStockError = (item) => {
        if (!item.productId || !item.quantity) return false;
        return parseInt(item.quantity) > getAvailableStock(item.productId);
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (validItems.some((item) => hasStockError(item))) return;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center p-6 border-b border-gray-200">
                    <h2 className="text-xl font-bold text-gray-800">Buat Sales Order</h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {items.map((item, index) => {
                                const stockError = hasStockError(item);
                                const availableStock = item.productId ? getAvailableStock(item.productId) : null;
                                    ...
                                    <div key={index}>
                                        <div className="grid grid-cols-12 gap-2 items-end">
                                            ...
                                            <div className="col-span-2">
                                                <input
                                                    ...
                                                    className={`input-field ${stockError ? 'border-red-500' : ''}`}
                                                    min="1"
                                                    placeholder="Qty"
                                                    required
                                                />
                                        ...
                                        {stockError && (
                                            <p className="text-xs text-red-500 mt-1 ml-1">
                                                Stok tidak cukup. Tersedia: {availableStock}
                                            </p>
                                        )}
                                        {!stockError && availableStock !== null && item.quantity && (
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
                        ...
                        <button
                            type="submit"
                            disabled={loading || items.some((item) => hasStockError(item))}
